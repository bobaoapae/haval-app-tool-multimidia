...
3. **Dynamic Themes (e.g. Minimalist - Downloaded OTA)**:
   ...
   - **Rule**: When requested to build or deploy a dynamic theme like Minimalist, agents **MUST**:
     ...
     3. **Commit and push** the updated source **and** the compiled `cluster-widgets/Themes/v1.0/minimalist/` assets (`app.html`, `theme.xml`, and any other package files) to the designated release branch (currently `bobaoapae/preview`) so it appears for in-app download and update detection.
